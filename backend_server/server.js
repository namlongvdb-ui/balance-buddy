...
app.post("/api/follow-up-analysis", async (req, res) => {
  try {
    const { conversationHistory, followUpQuestion, files } = req.body;

    if (!followUpQuestion?.trim() && (!files || files.length === 0)) {
      return res.status(400).json({ error: "Vui lòng nhập yêu cầu phân tích hoặc đính kèm file." });
    }

    // Trích xuất nội dung file nếu có
    let fileContent = "";
    if (files && files.length > 0) {
      console.log(`📂 Follow-up: Trích xuất ${files.length} file đính kèm...`);
      fileContent = await extractTextFromFiles(files, "follow-up");
    }

    const systemPrompt = `Bạn là chuyên gia kiểm toán và kế toán tài chính. Tiếp tục phân tích dựa trên ngữ cảnh cuộc hội thoại. 
NGUYÊN TẮC: CHỈ sử dụng dữ liệu đã được cung cấp trước đó và dữ liệu file đính kèm mới (nếu có), KHÔNG tự bịa số liệu. Giữ nguyên ngày tháng năm gốc.
Trả lời bằng tiếng Việt, format Markdown.`;

    let userMessage = followUpQuestion || "";
    if (fileContent) {
